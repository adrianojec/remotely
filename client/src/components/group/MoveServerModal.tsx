...
export const MoveServerModal: React.FC<MoveServerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#1e1e1e] border border-[#2b2b2b] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#2b2b2b] flex items-center justify-between bg-[#181818]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-100">Move Server to Group</h3>
              <p className="text-[11px] text-slate-400">{server.name} ({server.host})</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-2">Select Target Group</label>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl border text-xs text-left transition-all ${
                  selectedGroupId === null
                    ? 'bg-sky-950/40 border-sky-500/50 text-white font-medium shadow-sm'
                    : 'bg-[#181818] border-zinc-800 text-slate-300 hover:bg-zinc-800/40'
                }`}
              >
                <span>None (Ungrouped)</span>
                {selectedGroupId === null && <Check className="w-4 h-4 text-sky-400" />}
              </button>
