...
export const DeleteGroupModal: React.FC<DeleteGroupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#1e1e1e] border border-rose-500/30 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl transition-colors duration-200">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-200 dark:border-[#2b2b2b] flex items-center justify-between bg-rose-50 dark:bg-rose-950/20">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-500 dark:text-rose-400 flex items-center justify-center">
              <AlertTriangle className="w-4 h-4" />
            </div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Delete Server Group</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
            Are you sure you want to delete the group <strong className="text-slate-900 dark:text-white">"{group.name}"</strong>?
          </p>
          ...
          <div className="p-3.5 bg-rose-500/10 border border-rose-500/20 rounded-xl space-y-2">
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-normal">
              This action will permanently delete <strong className="text-rose-600 dark:text-rose-300">{containedServers.length} server(s)</strong> inside this group along with their connection details.
            </p>
            {containedServers.length > 0 && (
              <div className="pt-1.5 border-t border-rose-500/20 max-h-24 overflow-y-auto space-y-1">
                {containedServers.map((s) => (
                  <div key={s.id} className="text-[11px] font-mono text-slate-700 dark:text-slate-300 flex items-center justify-between">
                    <span className="truncate">• {s.name}</span>
                    <span className="text-slate-400 dark:text-slate-500 text-[10px] truncate ml-2">{s.host}</span>
                  </div>
                ))}
              </div>
            )}
