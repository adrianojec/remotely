...
export const RemoteDesktopView: React.FC<RemoteDesktopViewProps> = ({ server, onServerUpdated }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-100 dark:bg-[#121212] overflow-hidden rounded-xl border border-slate-200 dark:border-[#2b2b2b] relative transition-colors duration-200">
      {/* Top Controls Toolbar */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-slate-100 dark:bg-[#181818] border-b border-slate-200 dark:border-[#2b2b2b] select-none text-xs">
        {/* Left: Server info & Connection status */}
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-slate-900 dark:text-slate-100">{server.name}</span>
            <span className="px-2 py-0.5 rounded bg-sky-500/10 border border-sky-500/20 text-sky-600 dark:text-sky-400 text-[10px] font-mono">
              {currentProtocol}:{currentPort}
            </span>
          </div>

          {/* Connection Status Badge */}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-200/80 dark:bg-[#222] border border-slate-300/80 dark:border-[#333]">
            {connectionState === 'CONNECTING' && (
              <>
                <Loader2 className="w-3 h-3 text-amber-500 dark:text-amber-400 animate-spin" />
                <span className="text-amber-600 dark:text-amber-400 text-[11px] font-medium">Connecting...</span>
              </>
            )}
            {connectionState === 'CONNECTED' && (
              <>
                <span className="w-2 h-2 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
                <span className="text-emerald-600 dark:text-emerald-400 text-[11px] font-medium">Connected</span>
              </>
            )}
            {connectionState === 'DISCONNECTED' && (
              <>
                <span className="w-2 h-2 rounded-full bg-slate-400" />
                <span className="text-slate-500 dark:text-slate-400 text-[11px] font-medium">Disconnected</span>
              </>
            )}
            {connectionState === 'ERROR' && (
              <>
                <AlertCircle className="w-3 h-3 text-red-500 dark:text-red-400" />
                <span className="text-red-500 dark:text-red-400 text-[11px] font-medium">Error</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-200/80 dark:bg-[#222] border border-slate-300/80 dark:border-[#333] rounded-lg p-0.5">
            <button
              onClick={() => setScaleMode('fit')}
              className={`px-2 py-1 rounded text-[11px] font-medium transition-all ${
                scaleMode === 'fit' ? 'bg-sky-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
              }`}
            ...
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-medium transition-all ${
                scaleMode === '100' ? 'bg-sky-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
              }`}
            ...
            <button
              ...
              className={`px-2 py-1 rounded text-[11px] font-medium transition-all ${
                scaleMode === '125' ? 'bg-sky-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
              }`}
              title="125% Zoom"
            >
              125%
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-200/80 hover:bg-slate-300/80 dark:bg-[#222] dark:hover:bg-[#2e2e2e] border border-slate-300/80 dark:border-[#333] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all text-xs disabled:opacity-40"
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border transition-all ${
              isClipboardOpen
                ...
                : 'bg-slate-200/80 hover:bg-slate-300/80 dark:bg-[#222] dark:hover:bg-[#2e2e2e] border-slate-300/80 dark:border-[#333] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'
            }`}
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-200/80 hover:bg-slate-300/80 dark:bg-[#222] dark:hover:bg-[#2e2e2e] border border-slate-300/80 dark:border-[#333] text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all"
