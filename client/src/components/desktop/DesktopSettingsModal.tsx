...
export const DesktopSettingsModal: React.FC<DesktopSettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#1e1e1e] border border-slate-200 dark:border-[#333] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl transition-colors duration-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-[#2b2b2b] bg-slate-100 dark:bg-[#181818]">
          <div className="flex items-center gap-2.5 text-slate-900 dark:text-slate-100 font-semibold text-sm">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 dark:hover:text-white p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-white/5 transition-all"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {error && (
            <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 dark:text-red-400 text-xs leading-relaxed">
              {error}
            </div>
          )}
          ...
          {/* Protocol Selection */}
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Protocol</label>
              ...
              <button
                ...
                className={`py-2 px-3 rounded-xl border text-xs font-semibold flex items-center justify-center gap-2 transition-all ${
                  protocol === RdpProtocol.RDP
                    ...
                    : 'border-slate-200 dark:border-[#333] bg-slate-100 dark:bg-[#242424] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                RDP (Windows/Linux)
              </button>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-100 dark:bg-[#181818] border border-slate-200 dark:border-[#333] rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:border-sky-500"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
              Desktop Username <span className="text-slate-400 dark:text-slate-500 font-normal">(optional, defaults to SSH user)</span>
            </label>
            <input
              type="text"
              value={username}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
              Desktop Password <span className="text-slate-400 dark:text-slate-500 font-normal">(optional, leave blank to keep unchanged)</span>
            </label>
            <div className="relative">
              <input
                ...
                placeholder="••••••••••••"
                className="w-full bg-slate-100 dark:bg-[#181818] border border-slate-200 dark:border-[#333] rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-100 pr-10 focus:outline-none focus:border-sky-500"
              />
              <button
                type="button"
          ...
          {protocol === RdpProtocol.RDP && (
            <>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
                  Domain <span className="text-slate-400 dark:text-slate-500 font-normal">(optional Active Directory domain)</span>
                </label>
                <input
                  type="text"
                  value={domain}
          ...
          <div className="flex items-center gap-2 pt-1">
            <input
              ...
              className="rounded border-slate-300 dark:border-[#333] bg-slate-100 dark:bg-[#181818] text-sky-500 focus:ring-sky-500 focus:ring-offset-0"
            />
            <label htmlFor="ignoreCertCheck" className="text-xs text-slate-700 dark:text-slate-300 select-none cursor-pointer">
              Ignore self-signed SSL/TLS certificates
            </label>
          </div>

          {/* Actions */}
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-[#2b2b2b]">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-xl transition-all"
            >
              Cancel
            </button>
            <button
